...
var waves: Array[int] = [1, 2, 2, 3, 3, 5, 5]
...
func _ready() -> void:
	...
	Signals.connect_attack_boss(on_attack_boss)
	Signals.connect_damage_boss(on_damage_boss)
	setup_hp_bar()
...
func on_attack_boss(ant: Ant) -> void:
	ant.attack_boss(bosses_spawner.get_boss_position())
...
func setup_hp_bar() -> void:
	var boss_hp_max: int = GameState.bosses_hp[GameState.current_boss_index]
	boss_hp.max_value = boss_hp_max
	boss_hp.value = boss_hp_max


func fight_over(success: bool) -> void:
	if not is_battle_over:
		is_battle_over = true
...
func spawn_wave() -> void:
	...
	if wave_number >= waves.size():
		...
		return
	spawn_timer = 0.0
	var ants_count: int = waves[wave_number]
	wave_number += 1
	for i: int in range(ants_count):
	...
	if not boss_is_dead:
		if event is InputEventMouseButton and event.pressed:
			if event.button_index == MOUSE_BUTTON_LEFT:
				var event_pos: Vector2 = event.global_position
				ant_bite_spawner.spawn_ant_bite(event_pos, "random")
				Sfx.play_bite()
